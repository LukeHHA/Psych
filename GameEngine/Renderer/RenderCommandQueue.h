...
public:
  void Push(auto&& func)
  {
    m_CommandQueue_.push_back(std::forward<decltype(func)>(func));
  }
  ...
  {
    return m_CommandQueue_.rbegin();
  }
  const_reverse_iterator crbegin() const noexcept
  {
    return m_CommandQueue_.crbegin();
  }

  reverse_iterator rend() noexcept { return m_CommandQueue_.rend(); }
  const_reverse_iterator rend() const noexcept
  {
    return m_CommandQueue_.rend();
  }
  const_reverse_iterator crend() const noexcept
  {
    return m_CommandQueue_.crend();
  }
