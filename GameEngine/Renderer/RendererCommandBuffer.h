...
{
public:
  RendererCommandBuffer(const CommandBufferSpec& spec = CommandBufferSpec())
      : m_CommandBufferSpec_(spec)
  {
  ...
  void Push(auto&& func)
  {
    m_ActiveBuffer_->Push(std::forward<decltype(func)>(func));
  }
...
private:
  void SwapBuffers()
  {
    m_CurrentBufferIndex_ =
        (m_CurrentBufferIndex_ + 1) % m_CommandBufferSpec_.buffer_size;
    m_ActiveBuffer_ = &m_CommandQueues_.at(m_CurrentBufferIndex_);
  }
