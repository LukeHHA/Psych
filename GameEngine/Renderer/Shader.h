...
struct ShaderLibraryError : public errors::IErrors<ShaderLibraryErrors> {
  using errors::IErrors<ShaderLibraryErrors>::IErrors; // inherit ctor(s)

  virtual std::string ToString(ShaderLibraryErrors errorCode) const override
  {
    switch (errorCode) {
    case ge::ShaderLibraryErrors::NotFound:
      return "Shader was not found";
    default:
      return "Unknow Shader Library Error";
    }
  }
};
