                    ...
                    const std::string& filepath = "results.json")
  {
    ...
    if (m_CurrentSession) {
      ...
      if (Log::GetCoreLogger()) // Edge case: BeginSession() might be
                                // before Log::Init()
      {
        std::cout << "Instrumentor::BeginSession(" << name << ") when session"
    ...
    if (m_OutputStream.is_open()) {
      ...
      if (Log::GetCoreLogger()) // Edge case: BeginSession() might be
                                // before Log::Init()
      {
        CORE_LOG_ERROR("Instrumentor could not open results file '{0}'.",
                       filepath);
...
public:
  InstrumentationTimer(
      const char* name,
      PRINT_TRACE_CONTROL control = PRINT_TRACE_CONTROL::SUPPRESS_TRACE)
