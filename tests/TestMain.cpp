/*  EXAMPLE: Google Tests
    ...
    All assertion macros support streaming a custom failure message into them
   with the << operator, for example: EXPECT_TRUE(my_condition) << "My condition
   is not true";
*/
...
// If your assert throws exceptions instead, swap ASSERT_DEATH -> ASSERT_THROW
// accordingly.
...
TEST(EngineSingleton, GetReturnsSameInstance)
{
  ...
  ASSERT_EQ(&singleton, app.get())
      << "GameEngine::Get() did not return the active instance";
}
...
TEST(EngineLifecycle, DestructionResetsSingleton)
{
  ...
  ASSERT_TRUE(app2);
  ASSERT_NE(app2.get(), firstPtr)
      << "New engine instance reused same address (possible but unlikely)";
...
TEST(EngineSingleton, GetWithoutInstanceDies)
{
  ...
  ASSERT_DEATH({ (void)ge::GameEngine::Get(); }, "");
}
